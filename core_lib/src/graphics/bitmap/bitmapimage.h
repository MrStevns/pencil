...
public:
    ...
    /**
     * @brief applyTransparencyThreshold
     * The algorithm applies transparency threshold, removing all color information aside from alpha
     * @param maxThreshold - min threshold will always be 255-maxThreshold
     */
    static BitmapImage* applyTransparencyThreshold(BitmapImage* inputImage, int maxThreshold);
    BitmapImage* applyExpandPixels(BitmapImage* inputImage, int expandValue);
    int fillWithColor(BitmapImage* inputImage, QPoint point, QRgb orgColor, QRgb newColor);
    static BitmapImage* applyPixelatedLines(BitmapImage* inputImage);
