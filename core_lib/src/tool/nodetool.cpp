...
void NodeTool::pointerMoveEvent(PointerEvent* event)
{
    ...
    if (event->buttons() & Qt::LeftButton)   // the user is also pressing the mouse (dragging) {
    {
        if (event->modifiers() != Qt::ShiftModifier)    // (and the user doesn't press shift)
        {
            ...
            // qDebug() << offsetFromPressPosition;
            ...
            QRectF boundedTransformedRect = vectorImage->getBoundsOfTransformedCurves();
            blit.extend(boundedTransformedRect.toRect());

            // vectorImage->calculateSelectionRect();
            // vectorImage->setSelectionRect();
            // QRectF rect = vectorImage->getSelectionRect();
            // mEditor->select()->setSelection(boundedTransformedRect, false);
            // mEditor->select()->mySelectionRect()
...
void NodeTool::pointerReleaseEvent(PointerEvent* event)
{
